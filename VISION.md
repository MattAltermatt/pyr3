...
- **Affine + nonlinear variation chain** per xform, with optional per-xform post-affine +
  finalxform lens.
- **The flam3 variation library** — all 99 numbered VAR_* constants ported (V0..V98), plus the DC family + Apophysis + JWildfire 2D long tail + pyr3's own novel families (326 entries total, V0..V325 — the single-map strange-attractor trio V323 `sprott_poly` (#470 Sprott quadratic), V324 `hopalong` (#466 Barry Martin), and V325 `gumowski_mira` (#467) each ship with a CPU auto-search that seeds the Creator wall).
