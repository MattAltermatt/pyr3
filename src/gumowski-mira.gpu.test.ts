...
//
// #467 V325 gumowski_mira. Novel single-map attractor (no flam3-C ref). GPU smoke:
// matches the closed-form recursive rational map on sample coords + CPU↔GPU parity
// guard (ts_var_gumowski_mira ≈ var_gumowski_mira). G's denom 1+x² ≥ 1 → no NaN,
// no trig → no safe_* prelude. var_gumowski_mira calls gm_G, so the extracted
// prelude must include both fns.
import { afterAll, describe, expect, it } from 'vitest';
import { compileChecked } from './gpu-compile-guard';
import { extractWgslFn } from './shaders/extract';
import { acquireTestGpu, CHAOS_WGSL } from './gpu-test-harness';
import { ts_var_gumowski_mira, type VarInput } from './variations';
...
describe('var_gumowski_mira V325', () => {
  it('GPU matches CPU oracle on runtime args', async () => {
    ...
    const inputs: [number, number][] = [[0.3, -0.7], [-1.2, 0.4], [0.0, 0.9], [2.5, -1.1]];
    const gpu = await dispatchKernel('var_gumowski_mira', fnBody, inputs, `${a}, ${b}`);
    for (let i = 0; i < inputs.length; i++) {
      const inp: VarInput = { tx: inputs[i]![0], ty: inputs[i]![1], weight: 1, params: { a, b } };
      const o = ts_var_gumowski_mira(inp);
