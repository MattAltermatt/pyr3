...
async function main(): Promise<void> {
  ...
  // gate (always available; not user-facing). The console hook itself is
  // installed in installDevHooks (src/main-dev-hooks.ts, #423); the setter
  // side-effect (mutate + log) lives in setWalkerJitter below.
  //
  ...
  // Setter bridge for the DEV `__pyr3SetJitter` hook — mutates the real
  // binding above and logs, exactly as the old inline hook did.
  const setWalkerJitter = (amp: number): number => {
  ...
  // PYR3-018 / PYR3-027 — `lastRenderInfo` feeds the DEV `__pyr3CapturePixels`
  // / `__pyr3Bench` / `__pyr3Decoupled` console hooks (now installed in
  // installDevHooks, src/main-dev-hooks.ts, #423). It is written by the render
  // path below, so it stays a `let` here and is bridged into the hooks via a
  // getter.
  ...
  // Wire the bar's quality ladder to the render closure now that it's defined.
  // The dev-only `__pyr3RenderQuality` console hook is installed later in
  // installDevHooks (src/main-dev-hooks.ts, #423).
  ...
  // #423 — install ALL the DEV-only `window.__pyr3*` console + parity hooks in
  // one late call, now that every dependency (renderer, device, context,
  // format, sequencer, renderQuality, and the mutable `currentWalkerJitter` /
  // `seed` / `lastRenderInfo` / `runHandle` bindings) exists. No-op in
  // production builds (the function guards on import.meta.env.DEV, and the
  // call is dead-code-eliminated below). The getter/setter pairs mutate the
  // real `let` bindings above so the production render paths still read fresh
  // values.
  if (import.meta.env.DEV) {
    installDevHooks({
      renderer,
      ...
      sequencer,
      renderQuality,
      getJitter: () => currentWalkerJitter,
      setJitter: setWalkerJitter,
      getSeed: () => seed,
      setSeed: (v) => { seed = v; },
      getLastRenderInfo: () => lastRenderInfo,
      getRunHandle: () => runHandle,
      setRunHandle: (v) => { runHandle = v; },
    });
  }
