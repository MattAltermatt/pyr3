...
npm run render -- --no-auto-exposure <in> <out>             # disable render-time auto-exposure (#475). Auto-exposure is ON by default for display-referred output (png8/png16/exr): it renders a preview-res probe (1024px q16 = the viewer Preview tier), measures its mean luminance as the target, then scales `brightness` until the actual render hits that target — re-running only the cheap visualize/tonemap pass (no re-iterate). Fixes thin single-xform attractors (Gumowski-Mira/Hopalong) that render near-black at HQ/4K because a 1-D filament's per-bucket count grows with resolution+SPP, shrinking the log-density `ls`. A render-time VIEWING transform (like the color modes) — the embedded `pyr3` genome + source .pyr3.json keep their canonical authored `brightness`. Content-aware: a ±10% deadband makes well-exposed (area-filling) flames a no-op, so only genuinely-drifted flames move. `--no-auto-exposure` opts out (the BE↔flam3-C parity rig + FE↔BE harness pass it to measure the raw render); never applies to exr-linear (raw scene-linear histogram)
