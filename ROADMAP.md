...
1. **🚀 v1.0 — ship gate green + showcase + FE cleanup + GitHub repo
   replacement.** Four deliverables ship together as v1.0 (clarified
   2026-05-28 user-directives):
   - **Parity gates green** on the expanded 25-fixture corpus → CLAUDE.md
     decision #7 trigger pulled.
   - **Public showcase gallery** + **FE cleanup pass** — `[PYR3-007]`
     and `[PYR3-031]` ship as a **bundled pair** because they share the
     FE surface area. Start the session with a **brainstorm pass on the
     showcase shape** (no random-guessing); see `[PYR3-007]` for
     pre-discussed design directions (**unversioned URL** — no `/v1.0/`
     paths like kotlin; **render-time-not-comparison** — show pyr3 4K
     wall-clock per fixture but never compare against flam3/kotlin;
     **click-to-load as differentiator** — clicking a thumb loads the
     flame into the FE viewer at quick-mode; about blurb; permalinks;
     source-`.flame` download per fixture; mobile responsive). Then a
     coordinated implementation pass touches `src/main.ts` /
     `src/ui-bar.ts` / `src/render-orchestrator.ts` / `index.html`
     once instead of twice. Likely folds `[PYR3-020]` (share-link
     decode bug, blocks click-to-load).
   - **Push to `github.com/MattAltermatt/pyr3`** (replacing the kotlin
     repo); archive pyr3-kotlin and pyr3-peek per VISION
     (`§Acceptance`).

   Showcase URL: `https://mattaltermatt.github.io/pyr3/` (unversioned
   — always shows latest).
