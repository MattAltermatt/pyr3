...
- **`[PYR3-029]`** — chaos-walker-coverage parity audit (root cause of
  PYR3-017/021/024). 4 ranked sub-hypotheses to bisect: walker-pool seed
  dispersion → bad-iter rollback semantic → finalxform RNG draw order →
  color contraction across bad iters. **Gates 4K parity ship (PYR3-023)
  on most fixtures.**
- **`[PYR3-023]`** — promote `scripts/pyr3-023-be-render-4k.mjs` to a
  first-class CLI flag (`--preset showcase-4k`); build the BE 4K parity
  rig vs `fixtures/kotlin-4k-refs/`; calibrate R-thresholds against JPG
  noise floor. Step 1 (4096 → 3840 alignment) landed in v0.16. Remaining
  steps gated on `[PYR3-029]` landing so calibration is against a fixed
  engine.
